...
def read_mappings(lines):
	...
	for line in lines:
		if in_a_map and len(line) > 0:
			...
			mapping += [numbers]
		if in_a_map and len(line) == 0:
			mappings[mapping_name] = mapping_fn(mapping)
			in_a_map = False
		if "map" in line:
...
def mapping_fn(numbers_list):
	def inner(in_nb):
		for r in numbers_list:
			dst_start = r[0]
			src_start = r[1]
			range_len = r[2]
			if in_nb >= src_start and in_nb < src_start + range_len:
				return dst_start + in_nb - src_start
		return in_nb
...
for mapping in mapping_order:
	input_numbers = [mappings[mapping](nb) for nb in input_numbers]
