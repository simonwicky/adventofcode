...
for secret_number in data:
    ...
    for _ in range(2000):
        ...
        price_change += [price[-1] - price[-2]]
        secret_number = next_secret(secret_number)

    price = price[1:]
    price_change = price_change[1:]
    banana_amount = {}
    for i in range(3, len(price_change)):
        sequence = tuple(price_change[i-3:i+1])
        if sequence not in banana_amount:
            banana_amount[sequence] = price[i + 1]


    for sequence in banana_amount:
        total_bananas[sequence] = total_bananas.get(sequence,0) + banana_amount[sequence]
