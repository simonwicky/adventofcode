...
def pos_perimeter(plot, position):
    """return the perimeter part of position in plot (4 - number of neighbors)"""
    nb=4
    for direction in DIRECTIONS:
        if (position[0] + direction[0], position[1] + direction[1]) in plot:
            nb -= 1
    return nb
...
def plot_price(plot):
    ...
    area = len(plot)
    perimeter = sum([pos_perimeter(plot, position) for position in plot])
