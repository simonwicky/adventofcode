...
def parse_input(data):
    ...
    wires = {}
    emtpy_line = data.index("")

    for i in range(emtpy_line):
        wire = data[i].split(" ")[0][:-1]
        value = int(data[i].split(" ")[1])
        wires[wire] = value

    gates = {}
    for i in range(emtpy_line + 1, len(data)):
        gate = data[i].split(" ")
        op1 = gate[0]
        op = gate[1]
        op2 = gate[2]
        ...
        gates[(op2, op1, op)] = res
    return wires, gates


def check_bits(gates, A,B, S, C_in):
    A_xor_B = gates[(A,B,"XOR")]
    A_and_B = gates[(A,B,"AND")]
    tmp = gates[(A_xor_B, C_in, "AND")]

    if gates[(A_xor_B, C_in,"XOR")] != S:
        print("Mismatch for S")

    return gates[(tmp, A_and_B, "OR")]    


with open('input','r') as f:
    data = f.read().splitlines()

w,g = parse_input(data)

C_in = g[("x00","y00", "AND")]
for i in range(1,45):
    C_in = check_bits(g,f"x{i:02d}",f"y{i:02d}",f"z{i:02d}", C_in)

## I used the above code and then found and performs swaps manually whenever check_bits failed to find a key
