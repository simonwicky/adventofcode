...
#Horizontally keep track of inside/outside/on the loop
for i in range(len(ground_map)):
	inside = False
	on_loop = False
	for j in range(len(ground_map[i])):
		if (i,j) in pipe_loop:
			if ground_map[i][j] == '|':
				inside = not inside #crossing the loop
			elif not on_loop:
				on_loop = True
				symbol = ground_map[i][j] # getting on the loop
			elif on_loop and ((symbol == 'L' and ground_map[i][j] == '7') or (symbol == 'F' and ground_map[i][j] == 'J')): #leaving the loop, effectively crossing it
				on_loop = False
				inside = not inside
			elif on_loop and ((symbol == 'L' and ground_map[i][j] == 'J') or (symbol == 'F' and ground_map[i][j] == '7')): #leaving the loop, but no crossing
				on_loop = False
...
# 	for j in range(len(ground_map[i])):
# 		if (i,j) in pipe_loop:
# 			print(ground_map[i][j],end='')
# 		elif (i,j) in inside_p:
# 			print('*',end='')
# 		else:
# 			print('.',end='')
