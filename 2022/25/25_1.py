...
def decimal_to_snafu(decimal):
    ...
    while decimal != 0:
        ...
        if remainder > 2:
            remainder -= 5
            decimal += 1
        snafu.append('012=-'[remainder])
    return ''.join(reversed(snafu))

def parse_input(file_path):
    """Parse the input file and return a list of SNAFU numbers."""
    with open(file_path, 'r') as file:
        return [line.strip() for line in file.readlines()]

# Read input file
file_path = 'input'
snafu_numbers = parse_input(file_path)

# Convert SNAFU numbers to decimal and sum them up
total_decimal = sum(snafu_to_decimal(snafu) for snafu in snafu_numbers)

# Convert the total sum back to a SNAFU number
result_snafu = decimal_to_snafu(total_decimal)

print(result_snafu)  # Output the result